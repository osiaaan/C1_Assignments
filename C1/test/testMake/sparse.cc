  ...
  for(int i = 0 ; i < N_length_ ; ++i) // choosing row i
  {
    ...
    for(int j = 0 ; j < M_width_ ; ++j)
    {
      if(j == *it)//if there's a value, we print it
      {
        ...
        ro++;//and also move to the next value of row_i
      }
...
//This returns a vector containg the diagonal elements of the matrix
...
//It represents the L infinity norm
double infinityNorm(std::vector<double> x)
{
  double a = fabs(x[0]);
  for(int i = 1; i < x.size() ; ++i)
  {
      if(fabs(x[i]) > a) //comparing size
      {
        a = fabs(x[i]);
      }
  }
...
It will return a vector of vectors.
The first will be the approximate solution.
The second will be a vector consisting of the residual error
for each iteration in order.
Its size will be the number of iterations*/
